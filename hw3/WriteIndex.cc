...
static int WriteHeader(FILE* f, int doctable_bytes, int memidx_bytes) {
  ...
  // First use fseek() to seek to the right location
  // check whether seek position is at at the start of the table
  if (fseek(f, sizeof(IndexFileHeader), SEEK_SET) != 0) {
    return kFailedWrite;
  }
  uint8_t byte;
  for (int i = 0; i < (doctable_bytes + memidx_bytes); i++) {
    if (fread(reinterpret_cast<void *>(&byte),
        sizeof(uint8_t), 1, f) != 1) {
      return kFailedWrite;
    }
...
static int WriteHashTable(FILE* f, IndexFileOffset_t offset, HashTable* ht,
  ...
  for (int i = 0; i < ht->num_buckets; i++) {
    // STEP 4.
    // Writing BucketRecord into the disk
    int num_elts = LinkedList_NumElements(ht->buckets[i]);
    int br_bytes = WriteHTBucketRecord(f,
                                       record_pos,
                                       num_elts,
                                       bucket_pos);
    if (br_bytes != sizeof(BucketRecord)) {
      return kFailedWrite;
    }
    if (br_bytes == 0) {
      return kFailedWrite;
    }
    record_pos += br_bytes;  // update the record_pos

    // Writing Bucket contents into the disk
    int b_bytes = WriteHTBucket(f, bucket_pos, ht->buckets[i], fn);

    // Corner Case: The bucket chain is empty or not
    if ((b_bytes == 0 && num_elts > 0) || b_bytes < 0) {
      return kFailedWrite;
    }
    bucket_pos += b_bytes;  // update the bucket_pos
  }
...
static int WriteHTBucketRecord(FILE* f, IndexFileOffset_t offset,
  ...
  BucketRecord br;
  br.position = bucket_offset;
  br.chain_num_elements = num_elts;
  br.ToDiskFormat();
...
static int WriteHTBucket(FILE* f, IndexFileOffset_t offset, LinkedList* li,
  ...
  for (int i = 0; i < num_elts; i++) {
    ...
    ElementPositionRecord rec;
    rec.position = element_pos;
    rec.ToDiskFormat();
    ...
    int elmntfn = fn(f, element_pos, keyVal);
    if (elmntfn == 0) {
      LLIterator_Free(it);
      return kFailedWrite;
    }
...
static int WriteDocidToDocnameFn(FILE* f, IndexFileOffset_t offset,
  ...
  int16_t file_name_bytes = strlen(static_cast<char *>(kv->value));  // CHANGED
  ...
  if (fwrite(static_cast<void *>(kv->value), file_name_bytes, 1, f) != 1) {
    return kFailedWrite;
  }

  // STEP 11.
  // calculate and return the total amount written.
  return sizeof(DocIDElementHeader) + file_name_bytes;  // CHANGED
}
...
static int WriteDocIDToPositionListFn(FILE* f,
  ...
  // Now fseek() to the provided offset and then write the header.
  if (fseek(f, offset, SEEK_SET) != 0) {
    return kFailedWrite;
  }
  ...
  for (int i = 0; i < num_positions; i++) {
    ...
    LLPayload_t payload;
    LLIterator_Get(it, reinterpret_cast<void **>(&payload));

    // STEP 14.
    // Truncate to 32 bits, then convert it to network order and write it out.
    position.position = static_cast<uint32_t>
  ...
  return num_positions * sizeof(uint32_t) + sizeof(DocIDElementHeader);
}
...
static int WriteWordToPostingsFn(FILE* f,
  ...
  int16_t word_bytes = strlen(wp->word);  // CHANGED
