...
DocID_t DocTable_Add(DocTable* table, char* doc_name) {
  ...
  HTKey_t namehash = FNVHash64((unsigned char*) doc_name, strlen(doc_name));

  if (HashTable_Find(table->name_to_id, namehash, &old_kv)) {
  ...
  Verify333(doc_copy != NULL && doc_id != NULL);
...
DocID_t DocTable_GetDocID(DocTable* table, char* doc_name) {
  ...
  // return invalid docid if not found in hashtable
