...
static void HandleDir(char* dir_path, DIR* d, DocTable** doc_table,
  ...
  while (true) {
    ...
    // check for NULL (no elements left in d)
    if (dirent == NULL) {
      break;
    }
    ...
    if (stat(entries[i].path_name, &st) == 0) {
      ...
      if (S_ISREG(st.st_mode)) {
        // valid file so set is_dir attribute and move to the next entry
        ...
        // invalid element, so free the entry path_name string
...
static void HandleFile(char* file_path, DocTable** doc_table,
  ...
  // checkfor NULL
  // If tab is NULL, we can't build a hashtable out of the file
  if (tab == NULL) {
    return;
  }
  ...
  // check if doc_id gets an id or not
  Verify333(doc_id != 0);
  ...
  while (HTIterator_IsValid(it)) {
    ...
    // use MemIndex_AddPostingList() to add the word
    // document ID, and positions linked list into the inverted index.
