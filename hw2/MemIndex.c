...
void MemIndex_AddPostingList(MemIndex* index, char* word, DocID_t doc_id,
  ...
  if (!HashTable_Find(index, key, &mi_kv)) {
    ...
    Verify333(!HashTable_Insert((HashTable*) index, mi_kv, &unused));
...
LinkedList* MemIndex_Search(MemIndex* index, char* query[], int query_len) {
  ...
  // check memindex hash table for matches to query[0] word
  key = FNVHash64((unsigned char*) query[0], strlen(query[0]));
  if (HashTable_Find(index, key, &kv)) {
    // matches exist so allocate a retlist linked list
    ret_list = LinkedList_Allocate();
  } else {
    // no document matches exist so return NULL
    return NULL;
  }

  // loop through matching document postings
  wp = (WordPostings*) kv.value;
  HTIterator* iter = HTIterator_Allocate(wp->postings);
  while (HTIterator_IsValid(iter)) {
    // allocate a SearchResult struct for each posting
  ...
  for (i = 1; i < query_len; i++) {
    ...
    int elements = LinkedList_NumElements(ret_list);
    wp = (WordPostings*) kv.value;
    LLPayload_t payload;
    for (int i = 0; i < elements; i++) {
      ...
      if (!HashTable_Find(wp->postings, (HTKey_t) sr->doc_id, &kv)) {
        ...
        int occurrences = LinkedList_NumElements((LinkedList*) kv.value);
        sr->rank += occurrences;
