...
static void GetPortAndPath(int argc,
  ...
  // check port number is a valid uint16_t
  int p;
  if (!sscanf(argv[1], "%i", &p) || p < 1000 || p > 100000 ||
      !sscanf(argv[1], "%" SCNu16, port)) {
    std::cerr << "Invalid port number" << std::endl;
    Usage(argv[0]);
    return;
  }
  // check static directory path exists
  struct stat sb;
  if ((stat(argv[2], &sb) != 0) || access(argv[2], R_OK) == -1) {
    std::cerr << "Invalid static directory path" << std::endl;
    Usage(argv[0]);
    return;
  }
  ...
  for (int i = 3; i < argc; i++) {
    ...
    if (!infile.good() || access(argv[i], R_OK) == -1) {
      std::cerr << "Index file does not exist or is not readable" << std::endl;
      Usage(argv[0]);
      return;
    }
    // check file has .idx extension
    ...
    if (v.size() != 2 || v[1] != "idx") {
      ...
      // add valid index file to list
      indices->push_back(fpath);
    }
  }
