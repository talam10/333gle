...
bool IsPathSafe(const string& root_dir, const string& test_file) {
  ...
  // STEP 1
  // check whether there is a path to the directory
  // In this case we are using the realpath function
  // so that we get the absolute path and resultantly
  // we won't have to deal with "." and ".."
  char rootDBuf[PATH_MAX];
  char testFBuf[PATH_MAX];
  if ((!realpath(root_dir.c_str(), rootDBuf)) ||
      (!realpath(test_file.c_str(), testFBuf))) {
    return false;
  }

  // check whether the test_file is within the directory
  if (strlen(testFBuf) <= strlen(rootDBuf) ||
      strncmp(testFBuf, rootDBuf, strlen(rootDBuf)) != 0 ||
      testFBuf[strlen(rootDBuf)] != '/') {
    return false;
  }
...
string EscapeHtml(const string& from) {
  ...
  // STEP 2
  // will use the replace_all function of boost
  replace_all(ret, "&", "&amp;");
  replace_all(ret, "<", "&lt;");
  replace_all(ret, ">", "&gt;");
  replace_all(ret, "\"", "&quot;");
