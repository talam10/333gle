...
// Arguments:
// - kv_ptr: output parameter to return old key-value
// - remove: boolean determining whether an existing matching key is removed
// - key: specifies the key to remove
//
// Returns: boolean of whether a matching HTKeyValue_t was found.
bool findAndRemove(HashTable *table,
                          bool remove,
                          HTKey_t key,
                          HTKeyValue_t* kv_ptr);
...
// frees heap-allocated HTKeyValue_t payload in LL
static void LLOpFree(HTValue_t freeme) {
  free((HTKeyValue_t*) freeme);
}
...
bool findAndRemove(HashTable *table,
                          ...
                          HTKey_t key,
                          HTKeyValue_t* kv_ptr) {
  LinkedList* chain = table->buckets[HashKeyToBucketNum(table, key)];
  // create iterator for LL 'chain'
  LLIterator* iter = LLIterator_Allocate(chain);

  // loop through HT chain to find (and remove) 'key'
  while (LLIterator_IsValid(iter)) {
    // get the HTKeyValue payload from the LLIterator
    LLPayload_t payload;
    LLIterator_Get(iter, &payload);
    HTKeyValue_t keyvalue = *((HTKeyValue_t*) payload);

    // check if the node's key matches the target key
    if (keyvalue.key == key) {
      // copy node payload into output parameter
      *kv_ptr = keyvalue;

      // Remove node from the HT chain and decrement HT element counter
      if (remove) {
        LLIterator_Remove(iter, LLOpFree);
        table->num_elements--;
      }
...
bool HashTable_Insert(HashTable *table,
  ...
  int bucket = HashKeyToBucketNum(table, newkeyvalue.key);
  LinkedList* chain = table->buckets[bucket];

  // STEP 1: finish the implementation of InsertHashTable.

  // find and remove key with newkeyvalue if one exists
  bool keyFound = findAndRemove(table, true, newkeyvalue.key, oldkeyvalue);
  // insert pointer to newkeyvalue as address of a new LL_node in the HT chain
  HTKeyValue_t* kv_ptr = (HTKeyValue_t*) malloc(sizeof(HTKeyValue_t));
  *kv_ptr = newkeyvalue;
  LinkedList_Push(chain, (LLPayload_t) kv_ptr);
  // increment HT number of elements counter
  table->num_elements++;
  return keyFound;
}
...
bool HashTable_Find(HashTable *table,
  ...
  // remove argument is false because we only want to find, not remove
  return findAndRemove(table, false, key, keyvalue);
}
...
bool HashTable_Remove(HashTable *table,
  ...
  return findAndRemove(table, true, key, keyvalue);
}
...
bool HTIterator_IsValid(HTIterator *iter) {
  ...
  // invalid if the LLIterator for the current bucket is NULL
...
bool HTIterator_Next(HTIterator *iter) {
  ...
  // return false if the HTIterator is already invalid
  if (!(HTIterator_IsValid(iter))) {
    return false;
  }
  // advance the current bucket LLIterator if it's valid
  if (LLIterator_Next(iter->bucket_it)) {
    return true;
  } else {
    // else free the now invalid LLIterator
    LLIterator_Free(iter->bucket_it);
  }
  // advance through HT bucket array until the next non-empty linked list chain
  for (int i = (iter->bucket_idx + 1); i < iter->ht->num_buckets; i++) {
    // update bucket index and iterator if bucket points to non-empty LL chain
  ...
  // if there is no non-null linked list left in the HT bucket array,
  // set the iterator state to invalid and return false
  iter->bucket_it = NULL;
  return false;
}
...
bool HTIterator_Get(HTIterator *iter, HTKeyValue_t *keyvalue) {
  ...
  // store address of HTKeyValue_t in output parameter 'payload'
  LLPayload_t payload;
